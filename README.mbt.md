...
Compares two files (e.g. `README.md` ↔ `README-ja.md`, or the same README
before and after a change) and reports per-direction drift terms plus
cosine distance. Vocab sub-tokenization delegates to the natural-language
KGFs in `kgfs/natural/`, so cross-lingual pairs are first-class — Kanji /
Hiragana / Katakana / Hanzi / Hangul are segmented by their own lexers,
and POS-style stop classes (Article, Preposition, Joshi, Particle, …) are
excluded by token kind. A `--vocab-threshold` option turns drift detection
into a CI gate via exit code.
...
[Full documentation](cmd/indexion/plan/drift/README.md)
