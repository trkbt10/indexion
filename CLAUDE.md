...
### 絶対原則: indexionのコマンドを最初に使え

**コード調査・検索・探索はすべて indexion コマンドから始めろ。grep, find, Read への直行は禁止。**

```bash
# 関数・型・概念を探すとき → indexion search
moon run cmd/indexion --target native -- search "キーワード" 対象ディレクトリ/ --include='*.mbt'

# トークンパターンを探すとき → indexion grep
moon run cmd/indexion --target native -- grep "パターン" 対象ディレクトリ/

# 重複・類似を探すとき → indexion explore / plan refactor
```

indexionで見つからなかった場合、grep/Read に逃げるのではなく **indexionのコードを修正する。**
これはドッグフーディングに限らず、**このプロジェクトでの日常のコード調査すべて**に適用される。

### 問題に当たったら立ち止まれ

- indexionの出力が不完全・不正確だった → **その場でindexionのコードを修正する**
- 「とりあえずgrepで」は問題の先送りであり、ドッグフーディングの放棄である
- 問題を見つけること自体は成果だが、**修正せずに先に進むことは許されない**
